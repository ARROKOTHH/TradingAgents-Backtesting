...
class ChineseFinanceDataAggregator:
    ...
    def _search_finance_news(self, search_term: str, days: int) -> List[Dict]:
        """使用akshare搜索财经新闻"""
        try:
            # logger.info(f"🔍 使用akshare搜索关于 {search_term} 的财经新闻...")
            news_df = ak.stock_news_em(symbol=search_term)
            
            if news_df is None or news_df.empty:
                # logger.warning(f"⚠️ Akshare未能获取 {search_term} 的新闻。")
                return []
            ...
            for _, row in news_df.iterrows():
                news_items.append({
                    ...
                    'source': row.get('新闻来源', 'N/A'),
                    'publish_time': row.get('发布时间', ''),
                    'url': row.get('新闻链接', '')
                })
            
            # logger.info(f"✅ 成功从Akshare获取 {len(news_items)} 条关于 {search_term} 的新闻。")
            return news_items

        except Exception as e:
            # logger.error(f"❌ Akshare新闻搜索失败 for {search_term}: {e}")
            return []
