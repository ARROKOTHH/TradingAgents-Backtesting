...
def create_risk_manager(llm, memory):
    def risk_manager_node(state) -> dict:
        ...
        # 根据是否存在历史经验，动态生成提示
        if past_memory_str:
            learning_prompt = f"4. **从过去的错误中学习**：使用**{past_memory_str}**中的经验教训来解决先前的误判，改进您现在做出的决策，确保您不会做出错误的买入/卖出/持有决定而亏损。"
        else:
            learning_prompt = "4. **从过去的错误中学习**：目前没有过去的经验教训可供参考。请完全基于当前信息进行决策。"
