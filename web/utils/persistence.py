...
class ModelPersistence:
    ...
    def save_config(self, provider, category, quick_model, deep_model):
    ...
    def load_config(self):
        ...
        try:
            ...
            if 'provider' in query_params:
                config = {
                    ...
                    'quick_model': query_params.get('quick_model', ''),
                    'deep_model': query_params.get('deep_model', '')
                }
        ...
        if self.storage_key in st.session_state:
            ...
            # 兼容旧版配置
            if 'model' in config and 'quick_model' not in config:
                config['quick_model'] = config['model']
                config['deep_model'] = config['model']
                del config['model']
...
def save_model_selection(provider, category="", quick_model="", deep_model=""):
    """保存模型选择"""
    persistence.save_config(provider, category, quick_model, deep_model)
