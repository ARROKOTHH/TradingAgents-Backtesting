...
def run_stock_analysis(stock_symbol, analysis_date, analysts, research_depth, llm_provider, quick_think_llm, deep_think_llm, market_type="美股", progress_callback=None):
    """执行股票分析
    ...
    if TOKEN_TRACKING_ENABLED:
        ...
        estimated_cost = token_tracker.estimate_cost(llm_provider, deep_think_llm, estimated_input, estimated_output)
...
def validate_analysis_params(stock_symbol, analysis_date, analysts, research_depth, market_type="美股"):
    ...
    elif len(stock_symbol.strip()) > 20: # 放宽长度限制
        errors.append("股票代码长度不能超过20个字符")
    else:
        # 根据市场类型验证代码格式
