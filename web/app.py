...
<style>
    ...
    .analysis-section .streamlit-expanderContent {
        background-color: #262730 !important;
    }
    
    .success-box {
        background: #0d3a16;
        border: 1px solid #1a6e2c;
        border-radius: 5px;
        padding: 1rem;
        margin: 1rem 0;
        color: #FAFAFA;
...
def main():
    ...
    page = st.sidebar.selectbox(
        "切换功能模块",
        ["📊 股票分析", "📈 策略生成与回测", "⚙️ 配置管理", "💾 缓存管理", "💰 Token统计", "📈 历史记录", "🔧 系统状态"],
    ...
    st.session_state.config = config # 将配置保存到session_state

    # 根据选择的页面渲染不同内容
    if page == "📈 策略生成与回测":
        try:
            from web.modules.strategy_backtesting import render_strategy_backtesting_page
            render_strategy_backtesting_page(config)
        except ImportError as e:
            st.error(f"策略生成与回测模块加载失败: {e}")
        return
    elif page == "⚙️ 配置管理":
        try:
            ...
            if not is_valid:
                ...
                def run_analysis_in_background():
                    try:
                        ...
                        async_tracker.mark_failed(detailed_error)
                        logger.error(f"❌ [分析失败] {analysis_id}: {detailed_error}")
