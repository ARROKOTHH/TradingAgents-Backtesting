...
"""
    ...
    # 新增：提供回测错误信息的输入区域
    with st.expander("高级：提供回测错误以生成修复代码", expanded=False):
        st.markdown("如果您在回测时遇到了错误，可以将错误信息粘贴到下方，并勾选复选框，然后重新点击“生成策略”按钮，AI将尝试基于错误信息修复代码。")
        backtest_error_for_fix = st.text_area("(可选) 粘贴回测错误信息以帮助AI修复代码:", key="backtest_error_for_fix")
        use_backtest_error_for_fix = st.checkbox("使用提供的错误信息生成修复代码", value=False, key="use_backtest_error_for_fix")
        ...
        with st.spinner("正在调用AI工程师生成代码，AI将自动修正语法错误..."):
            try:
                ...
                for i in range(max_retries):
                    ...
                    if i == 0:
                        # 检查是否使用回测错误进行修复
                        if use_backtest_error_for_fix and backtest_error_for_fix.strip():
                            # 使用回测错误修复的Prompt
                            prompt = f"""
您是一位顶级的、精通`backtrader`框架的量化策略工程师。您上次生成的策略代码在回测时出现了错误，请根据错误信息进行修复。原始策略要求和最终风险偏好不变。
...
---
**【极其重要的警告】**
以下错误是绝对不能犯的，如果您的代码中出现这些错误，将会被严厉批评并要求重新生成：
1.  **绝对禁止**: 直接访问多线指标的属性，如 `macd.histo`, `bbands.top` 等。
2.  **必须使用**: 通过 `.lines` 属性访问，如 `macd.lines.histo`, `bbands.lines.top` 等。

**【强制修复指令】**
请仔细检查代码中所有对 `backtrader` 指标属性的访问。对于像 `MACD`, `BollingerBands`, `ADX` 这样的多线指标，必须使用 `.lines` 属性来访问其子线。
- 错误示例: `self.macd.histo`, `self.bband.top`, `self.dmi.DIplus`
- 正确示例: `self.macd.lines.histo`, `self.bband.lines.top`, `self.dmi.lines.DIp`
...
"""
                    ...
                    print(f"[DEBUG] AI生成的原始代码 (第 {i + 1} 次尝试):\n{raw_code}\n")
                    st.session_state.strategy_code = auto_correct_backtrader_code(raw_code)
                    print(f"[DEBUG] AI生成并修正后的代码 (第 {i + 1} 次尝试):\n{st.session_state.strategy_code}\n")
                    ...
                    try:
                        ...
                        # 根据是否是修复代码，决定文件名后缀
                        if use_backtest_error_for_fix and backtest_error_for_fix.strip():
                            strategy_filename = f"strategy_{stock_symbol}_{timestamp}_fixed.py"
                        else:
    ...
    if st.session_state.get('backtest_results') and st.session_state.get('strategy_code'):
        ...
        with st.form(key="chat_form", clear_on_submit=True):
            ...
            user_input = st.text_area("向策略分析师提问或提出修改建议:", key="user_chat_input", height=100)
            col1, col2 = st.columns([1, 1])
            with col1:
                submit_button = st.form_submit_button("发送")
            with col2:
                clear_button = st.form_submit_button("清除对话历史")
...
"""
        ...
        if st.button("导出对话历史"):
            ...
            for msg in st.session_state.chat_history:
                if msg["role"] == "user":
                    chat_history_text += f"## 您:\n{msg['content']}\n\n"
                else:
                    chat_history_text += f"## 策略分析师:\n{msg['content']}\n\n"
            
            # 提供下载链接
            st.download_button(
                label="下载对话历史",
                data=chat_history_text,
                file_name=f"strategy_chat_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md",
                mime="text/markdown"
            )