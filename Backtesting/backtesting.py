...
def get_data(symbol, start_date, end_date, source="A股", adjust="qfq"):
    ...
    try:
        if source == "A股":
            ...
            print(f"获取上海黄金交易所 '{symbol}' 的历史行情数据...")
            df = ak.spot_hist_sge(symbol=symbol)
            print("SGE 历史行情原始数据列名:", df.columns.tolist())
...
# ================= 回测执行函数 =================
def run_backtest(file=None, use_akshare=False, symbol=None, start_date=None, end_date=None, source='A股', adjust='qfq',
                 fast_ma=5, slow_ma=20, long_ma=200, atr_period=14, stop_loss_atr=2.0, 
                 take_profit_1=0.3, take_profit_2=0.5, pyramid_add=0.1,
                 use_custom_strategy=False, strategy_file=None, initial_cash=100000):
        ...
        if use_custom_strategy and strategy_file:
            ...
            try:
                ...
                if hasattr(custom_module, 'CustomStrategy'):
                    original_strategy_class = getattr(custom_module, 'CustomStrategy')
                    print(f"成功加载自定义策略: {original_strategy_class.__name__}")
                    
                    # 动态包装 CustomStrategy 以修复 MACD 的 histo 问题
                    class WrappedCustomStrategy(original_strategy_class):
                        def __init__(self):
                            super().__init__()
                            # 检查是否使用了 MACD 且缺少 histo
                            if hasattr(self, 'macd') and not hasattr(self.macd.lines, 'histo'):
                                print("检测到 MACD 指标缺少 histo，自动添加 histo 计算")
                                # 使用 MACDHistogram 替换 MACD
                                self.macd = bt.indicators.MACDHistogram()
                                # 重新定义 macd_crossover 以确保一致性
                                self.macd_crossover = bt.indicators.CrossOver(self.macd.lines.macd, self.macd.lines.signal)
                    
                    strategy_class = WrappedCustomStrategy
                    print(f"已包装策略为: {strategy_class.__name__}")
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        source = gr.Dropdown(["A股", "美股", "黄金现货"], label="数据源", value="A股")
